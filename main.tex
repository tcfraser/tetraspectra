...
\begin{rem}
    From the definition of $\varphi^{\ya\yb}_{\yc}(X,Y)$, one concludes that for all $\ya,\yb$:
    \begin{equation}
    ...
    \end{equation}
    where the sum is over all $\yc$ such that $\syc = \sya + \syb$.
    However, an analogous identity for the operator $\Phi^{\ya\yb}_{\yc}(X,Y)$ does not immediately follow because the tensor product $\Phi_{\ya}(X) \ot \Phi_{\yb}(Y)$ is not necessarily block diagonalizable into $\yc$-isotypic subspaces.
\end{rem}
...
\begin{cor}
    Let $X, Y \geq 0$ be positive semidefinite $n\times n$ matrices.
    Then for all Young diagrams $\yc$, 
    \begin{equation}
        \varphi_{\yc}(X+Y) \leq \sum_{\substack{\ya,\yb\\c^{\yc}_{\ya\yb} > 0}}\varphi_{\ya}(X)\varphi_{\yb}(Y).
    \end{equation}
    Similarly, for all Young diagrams $\ya$ and $\yb$,
    \begin{equation}
        \varphi_{\ya}(X)\varphi_{\yb}(Y) \leq \sum_{\substack{\ya,\yb\\c^{\yc}_{\ya\yb} > 0}}\varphi_{\yc}(X+Y).
    \end{equation}
\end{cor}
...
\begin{prop}
    ...
    Then
    \begin{align}
        \sum_{\yc\ye\yf}\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z) 
        &= \varphi_{\ya}(X)\varphi_{\yb}(Y)\varphi_{\yd}(Z) \\
        \sum_{\ye\yf}\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z) 
