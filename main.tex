...
Moreover, even under the assumption that $X,Y,Z \geq 0$ are positive semidefinite, unlike the quantitity $\varphi^{\ya\yb}_{\yc}(X,Y)$, the quantity $\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)$ is not necessarily nonnegative or even real valued.
This is because the projection operators $P^{\yc}_{\ya\yb}$ and $P^{\yf}_{\yb\yd}$ do not necessarily commute with each other in the sense that for some values of $\ya,\yb,\yd,\yc,$ and $\yf$, one observes
\begin{equation}
...
\end{equation}
Therefore, can can define Hermitian projection operators $\Pi_{\ya\yb\yd}^{\ye;\yc}$ and $\Pi_{\ya\yb\yd}^{\ye;\yf}$ by
\begin{align}
...
\end{equation}
Moreover we denote parital sums of $\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)$ over $\yc$ and $\yf$ respectively as
\begin{align}
...
\begin{equation}
    \varphi^{\ya\yb\yd}_{\_\ye\yf}(X,Y,Z) \geq 0, \qquad \varphi^{\ya\yb\yd}_{\yc\ye \_}(X,Y,Z) \geq 0.
\end{equation}

We summarize all of the summation identities satisfied by $\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)$ in the following diagram:
...
\end{proof}
In preparation of applying \cref{lem:cs_ineq_double_proj}, we observe the following.
\begin{prop}
    \label{prop:6j_norm_double_proj}
    For any sextuple of Young diagrams $(\ya,\yb,\yc,\yd,\ye,\yf)$,
    \begin{equation}
        \norm{\abcdefsymbol}_{\infty} = \norm{\Pi_{\ya\yb\yd}^{\ye;\yc}\Pi_{\ya\yb\yd}^{\ye;\yf}}_{\infty},
    \end{equation}
...
\begin{lem}
    ...
    Let $(x,y,z)$ be a triple of positive semidefinite eigenvalues of $n \times n$ matrices and let $X \sim \mathcal O_{x}, Y \sim \mathcal O_{y},$ and $Z \sim \mathcal O_{z}$ be random positive semidefinite matrices distributed independently and Haar-uniformly in their respective coadjoint orbits.
    Then
    \begin{equation} 
        \expect_{X \sim \mathcal O_{x}} \expect_{Y \sim \mathcal O_{y}} \expect_{Z \sim \mathcal O_{z}} \frac{\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)}{\dim V_{\ye}^{n}} = \norm{\abcdefsymbol}_{2}^{2} \frac{\varphi_{\ya}(x)}{\dim V_{\ya}^{n}}\frac{\varphi_{\yb}(y)}{\dim V_{\yb}^{n}}\frac{\varphi_{\yd}(z)}{\dim V_{\yd}^{n}}.
    \end{equation}
\end{lem}
...
\begin{thm}
    ...
    \end{equation}
    Moreover, if $(\ea,\eb,\ec,\ed,\ee,\ef)$ satisfies the trace conditions and \cref{eq:nec_suff_tet} for all triples $(\ya,\yb,\yd)$ such that $\sya + \syb + \syd = k$ for some positive integer $k$, then
    \begin{equation}
        D\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef} \coloneqq \min \max\{ \norm{}_1, \norm{}_1, \norm{}_1 \}
    \end{equation}
\end{thm}
\begin{proof}
    The necessity of \cref{eq:nec_suff_tet} is simply a restatement of \cref{cor:tet_eigenvalue_cubic} under the identification $A = X, B = Y,$ and $D = Z$.
    All that remains therefore is to prove the distance bound for the case where \cref{eq:nec_suff_tet} is satisfied for all triples $(\ya,\yb,\yd)$ such that $\sya + \syb + \syd = k$.
    If a sextuple of eigenvalues $(\ea,\eb,\ec,\ed,\ee,\ef)$ happens to satisfy \cref{eq:nec_suff_tet} for a particular triple of Young diagrams $(\ya,\yb,\yd)$, then multiplying both sides of \cref{eq:nec_suff_tet} by $\varphi_{\ya}(a)^{\frac{4}{3}}\varphi_{\yb}(b)\varphi_{\yd}(d)^{\frac{4}{3}}$ yields
    \begin{equation}
    ...
    \end{equation}
    Now we intend to sum this expression over all triples $(\ya,\yb,\yd)$ such that $\sya + \syb + \syd = k$.
    For convenience, let $W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}$ denote the corresponding sum of the right-hand side:
    \begin{equation}
        W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}
        \coloneqq \sum_{\substack{\ya,\yb,\yd \\ \yc,\ye,\yf \\ \sye = k}} \norm{\abcdefsymbol}_{\infty} 
        \varphi_{\ya}(a)^{\frac{4}{3}}\varphi_{\yb}(b)\varphi_{\yd}(d)^{\frac{4}{3}} \varphi_{\yf}(f)^{\frac{1}{3}}\varphi_{\yc}(c)^{\frac{1}{3}}\varphi_{\ye}(e)^{\frac{1}{3}}.
    \end{equation}
    Here we have used the fact that $\norm{\abcdefsymbol}_{\infty}$ is only non-zero if $\sya + \syb + \syd = \sye = k$.
    Now we remark that summing over $\varphi_{\ya}(a)\varphi_{\yb}(b)\varphi_{\yd}(d)$ yields
    \begin{equation}
    ...
    Therefore, if $\Lambda_k^{(a,b,d)}$ denotes the number of triples $(\ya,\yb,\yd)$ such that $\varphi_{\ya}(a)\varphi_{\yb}(b)\varphi_{\yd}(d) \neq 0$, then standard moment inequalities produce:
    \begin{align}
        W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef} \geq \frac{1}{\Lambda_k^{(a,b,d)}}\left(\frac{\tee^k}{k!}\right)^2.
    \end{align}
    Our objective now is to derive a complementary upper bound on $W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}$ which involves the distance measure $D\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}$ using the eigenvalue separation lemma.
    To accomplish this, note that an immediate corrollary of \cref{lem:random_matrices_tet} is the following bound which holds for any triple $(x,y,z)$ of positive semidefinite eigenvalues:
    \begin{equation} 
        \norm{\abcdefsymbol}_{2}^{2} \varphi_{\ya}(x)\varphi_{\yb}(y)\varphi_{\yd}(z) \leq R^{\ya\yb\yd}_{\ye} \max_{\substack{(X,Y,Z) \\\in \mathcal O_x \times \mathcal O_y \times \mathcal O_z}}\abs{\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)},
    \end{equation}
    where $R^{\ya\yb\yd}_{\ye} = \frac{\dim (V_\ya^{n} \ot V_{\yb}^{n} \ot V_{\yd}^{n})}{\dim (V_{\epsilon}^{n})}$.
    Applying \cref{thm:varphi_tet_main_ineq} (and its related corollaries) to $\abs{\varphi^{\ya\yb\yd}_{\yc\ye\yf}(X,Y,Z)}$ on the right-hand side above produces the following:
    \begin{equation} 
        \label{eq:sup_bound}
        \norm{\abcdefsymbol}_{\infty} \varphi_{\ya}(\ex)^{\frac{2}{3}}\varphi_{\yb}(\ey)\varphi_{\yd}(\ez)^{\frac{2}{3}} \leq R^{\ya\yb\yd}_{\ye} \max_{\substack{(X,Y,Z) \\\in \mathcal O_\ex \times \mathcal O_\ey \times \mathcal O_\ez}} \varphi_{\yf}(Y+Z)^{\frac{1}{3}}\varphi_{\yc}(X+Y)^{\frac{1}{3}}\varphi_{\ye}(X+Y+Z)^{\frac{1}{3}}.
    \end{equation}
    ...
    Then \cref{eq:sup_bound} can be applied to each summand in $W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}$ (making sure to replace $(\ex,\ey,\ez)$ with $(\ea,\eb,\ed)$) to obtain
    \begin{equation} 
        W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}
        \leq 
        \sum_{\substack{\ya,\yb,\yd \\ \yc,\ye,\yf \\ \sye = k}} R^{\ya\yb\yd}_{\ye} \varphi_{\ya}(a)^{\frac{2}{3}}\varphi_{\yd}(d)^{\frac{2}{3}}\max_{\substack{(X,Y,Z) \\\in \mathcal O_\ea \times \mathcal O_\eb \times \mathcal O_\ed}} \left[\varphi_{\yf}(\ef)\varphi_{\yf}(Y+Z)\varphi_{\yc}(\ec)\varphi_{\yc}(X+Y)\varphi_{\ye}(\ee)\varphi_{\ye}(X+Y+Z)\right]^{\frac{1}{3}}.
    \end{equation}
    Now the eigenvalue separation lemma applied to each summand gives
    \begin{align} 
        W_k\!\tightparensixj{\ea & \eb & \ec \\ \ed & \ee & \ef}
        &\leq 
        \sum_{\substack{\ya,\yb,\yd \\ \yc,\ye,\yf \\ \sye = k}}
        R^{\ya\yb\yd}_{\ye} \dim (V_{\yc}\ot V_{\ye}\ot V_{\yf})^{\frac{2}{3}} \left(\frac{\tea^{\sya}}{\sya!}\frac{\ted^{\syd}}{\syd!}\frac{\tilde F(c, \eig(X+Y))^{\syc}}{\syc!}\frac{\tilde F(e, \eig(X+Y+Z))^{\sye}}{\sye!}\frac{\tilde F(f, \eig(Y+Z))^{\syf}}{\syf!}\right)^{\frac{2}{3}}
    \end{align}
\end{proof}
...
\begin{prop}
    ...
    \end{equation}
    where $\dim V_{\yx}^{n} = s_{\yx}(e)$ counts the number of semistandard Young tableaux and therefore
    \begin{equation}
        \frac{k!}{\tex^{k}}\varphi_{\yx}(\ex) = p_{X}(\yx) \leq s_{\yx}(e) m_{\nrm \ex}(\yx)
    \end{equation}
...
\begin{prop}[Eigenvalue separation]
    ...
    \end{equation}
\end{prop}
\begin{prop}[Eigenvalue separation fixed $\yx$]
    Let $X,Y\geq 0$ be positive semidefinite $n\times n$ matrices with the same trace $\tau = \Tr(X) = \Tr(Y)$.
    Then for any Young diagram $\yx \vdash k$,
    \begin{equation}
        \sqrt{\varphi_{\yx}(X)\varphi_{\yx}(Y)} \leq s_{\yx}(e)\frac{\tilde F(\ex,\ey)^k}{k!} .
    \end{equation}
    where $\tilde F(\ex,\ey) \coloneqq \tau F(\nrm \ex,\nrm \ey)$ denotes the unnormalized fidelity between $\ex$ and $\ey$ (it is strictly less that $\tau = \tex = \tey$ when $\ex \neq \ey$ and equals $\tau$ otherwise).
\end{prop}
\begin{rem}
    Note that the fidelity $F(p,q)$ between distributiosn $p,q$ is related to the total variation distance $TV(p,q) = \frac{1}{2}\norm{p-q}_{1}$ via the inequalities:
    \begin{equation}
        1- TV(p,q) \leq F(p,q) \leq \sqrt{1-TV(p,q)^2},
    \end{equation}
    or alternatively, the Bhattacharyya coefficient $B(p,q) = - \log F(p,q)$ has $B(p,q) \geq \frac{1}{2} TV(p,q)^2 = \frac{1}{4}\norm{p-q}_1^2$.
\end{rem}
